...
const Womens = () => {
    ...
    useEffect(() => {
        fetch('https://tranquil-inlet-80790.herokuapp.com/womens/shirts')
        ...
        fetch('https://tranquil-inlet-80790.herokuapp.com/womens/tshirts')
            .then((response) => response.json())
            .then((data) => {
                setTShirtsArray(data);
            })

        fetch('https://tranquil-inlet-80790.herokuapp.com/womens/pantsAndTrousers')
            .then((response) => response.json())
            .then((data) => {
                setPantsArray(data);
            })
    ...
    const iWillHandleWomensData = (section) => {
    ...
    if (shirtsArray.length > 0 && tShirtsArray.length > 0 && pantsArray.length > 0) {
        return (
            <Grid container direction='row' justify='space-between' alignItems='center'>
                <Grid item xs={2} sm={2}>
                    <DrawerCustom womens='true' handleWomensSection={(section) => iWillHandleWomensData(section)} />
                </Grid>
